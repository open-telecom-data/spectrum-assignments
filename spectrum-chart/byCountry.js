...
function displayAssignments(band, bandStart, bandEnd, guardStart, guardEnd) {
    ...
    d3.csv('csv/freqAssignment_sql.csv', function(error, freqData) {
        ...
        d3.csv('csv/spectrumLicense_sql.csv', function(error, licData) {
            /* Load data from operators.csv file */
            d3.csv('csv/operators_sql.csv', function(error, opData) {
                /* Load data from operators.csv file */
                d3.csv('csv/owner_sql.csv', function(error, ownData) {
                    /* Load data from operators.csv file */
                    d3.csv('csv/shareHolding_sql.csv', function(error, shareData) {
                        /* Load data from countries.csv file */
                        d3.csv('csv/country.csv', function(error, countryData) {
                            /* Load data from freqBands.csv file */
                            d3.csv('csv/freqBands_sql.csv', function(error, bandData) {
                                ...
                                bandData.forEach(function (d) {
                                    d.ID = +d.ID;
                                    d.bandStart = +d.bandStart;
                                    d.bandEnd = +d.bandEnd;
                                    d.guardStart = +d.guardStart;
                                    d.guardEnd = +d.guardEnd;
                                });


                                /* perform join of countries to Operators */
                                var OperatorCountryJoin = join(countryData, opData,"ID", "Country_ID", function (op, cnt) {
                                    return {
                                        Operator_ID:  op.ID,
                                ...
                                var LicenseCountryJoin_tmp = join(OperatorCountryJoin, licData,"Operator_ID", "Operator_ID", function (lic, op) {
                                    return {
                                        ID:  lic.ID ,
                                ...
                                var LicenseCountryJoin = join(bandData, LicenseCountryJoin_tmp,"ID", "Band_ID", function (lic,bnd) {
                                    return {
                                        ID:  lic.ID ,
                                        Country: lic.Country,
                                        ISO: lic.ISO,
                                ...
                                var licenseJoin = join(LicenseCountryJoin, freqData, "ID", "license_ID", function (freq, lic) {
                                    return {
                                ...
                                var operatorJoin = join(OperatorCountryJoin, licenseJoin, "Operator_ID", "Operator_ID", function (licJ, op) {
                                    return {
                                ...
                                y.domain(freqDataBand.map(function (d) {
                                    return d.Country;
                                }));
                                ...
                                h.selectAll(".guardbands")
                                    ...
                                    .attr("y", function (d) {
                                        return x(guardStart) + (x(guardEnd) - x(guardStart)) / 2 + 5;
                                    })
                                    .attr("x", function (d) {
                                        return -y(d.Country) - y.bandwidth() + 10;
                                    })
                                    .text(guardBand)
                                ...
                                h.selectAll(".bar")
                                    ...
                                    .attr("x", function (d) {
                                        return x(d.freqStart);
                                    })
                                    .attr("width", function (d) {
                                        return x(d.freqEnd) - x(d.freqStart);
                                    })
                                    .attr("height", y.bandwidth());
                                ...
                                bars.append("text")
                                    ...
                                    .attr('transform', 'rotate(-90)')
                                    .attr("y", function (d) {
                                        return x(d.freqStart) + (x(d.freqEnd) - x(d.freqStart)) / 2 + 5;
                                    })
                                    .attr("x", function (d) {
                                        return -y(d.Country) - y.bandwidth() + 10;
                                    })
                                ...
                                $('#myModal').on('show.bs.modal', function () {
                                    ...
                                    let freqDataOperator = freqDataBand.filter(function (d) {
                                        return e.Operator_ID == d.Operator_ID
                                    });
                                    let sharesHeld = ownership.filter(function (d) {
                                        return e.Operator_ID == d.Operator_ID
                                    });
                                    let freqsHeld = operatorJoin.filter(function (d) {
                                        return e.Operator_ID == d.Operator_ID
                                    });
